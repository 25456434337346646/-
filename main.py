...
@register("astrbot_plugin_multimodal_pdf_router", "Anti-Gravity Agent", "基于‘视觉中转’链路的深度解析插件", "1.9.4")
class MultimodalPDFRouterPlugin(Star):
    ...
    async def _render_pdf(self, html_body: str, model_name: str) -> str:
        ...
        mathjax_script = """<script>window.MathJax={tex:{inlineMath:[['$','$'],['\\\\(','\\\\)']],displayMath:[['$$','$$'],['\\\\[','\\\\]']]},startup:{pageReady:()=>MathJax.startup.defaultPageReady().then(()=>window.MATHJAX_DONE=true)}};</script><script id="MathJax-script" src="https://npm.elemecdn.com/mathjax@3.2.2/es5/tex-mml-chtml.js"></script>"""
    ...
    async def handle_multimodal_query(self, event: AstrMessageEvent):
            ...
            if isinstance(comp, Plain):
                ...
                try:
                    ...
                    if isinstance(msg_data, dict):
                        msg_paths = [ ["message"], ["data", "message"], ["data", "messages"], ["messages"], ["content"] ]
                        for path in msg_paths:
                            temp_data = msg_data
                            try:
                                for key in path:
                                    temp_data = temp_data.get(key)
                                    if temp_data is None: break
        ...
        try:
            if question:
                logger.info(f"[知识库增强] 正在检索: {question[:50]}")
                # 修复BUG: 移除意外关键字 kb_ids
            ...
            logger.warning(f"[知识库增强] 检索异常: {kb_e}")
        ...
        try:
            pdf_path = await self._render_pdf(pdf_content, text_model)
